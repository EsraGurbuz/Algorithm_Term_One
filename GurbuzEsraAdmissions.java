/*******************************************************************
@Title:	    GurbuzEsraAdmissions
@Purpose:	To get familiar with file IO and writing methods
@Author:    Gurbuz  Esra
@Date:   	11.27.2024 27th of November 2024
@Version:	1.0
********************************************************************/
...
public class GurbuzEsraAdmissions {
    ...
    public static void information(String infoFileName) throws IOException {
        ...
        outputFile.println("Sabahattin Ali 20 3000");
        outputFile.println("Gollum smeagol 10 700");
        outputFile.close();
	...
	public static void checkTheStatement(String infoFileName, String resultFileName) throws IOException {
		...
		PrintWriter outputFile = new PrintWriter(new FileWriter(resultFileName, true));
		...
		while (inputFile.hasNextLine()) {
			...
			if (!validAct(actScore) || !validSat(satScore)) {
				outputFile.println(firstName + " " + lastName + " has invalid value. Check your input.");
				continue;
			}
